...
bool sim(Graph& G, Path &p1, Path &p2, double threshold) {
    ...
    for (int i=0;i<n1;i++) {
        for (int j=0;j<n2;j++) {
            if (G.adj[p1.vertices[i]->getid()][p1.vertices[i+1]->getid()] == G.adj[p2.vertices[j]->getid()][p2.vertices[j+1]->getid()]) {
                count += 1;
            }
        }
    }
...
std::vector<Path> KSP_heuristic(Graph &G, Node* start, Node* dest, int &k, double threshold, double& total_penalty) {
    ...
    while (lowest.size()<k) {
        ...
        while (!h.empty()) {
            ...
            if (!found) {e->updateRestriction(true);}
    ...
    for (int i=0;i<lowest.size();i++) {
        ...
        for (int j=i;j<lowest.size();j++) {
            if (j==i) {continue;}
            if (!sim(G,lowest[i],lowest[j],threshold)) {overlap++;}
        }
